...
def INT():
    ...
    class KINT(tf.keras.Model):
        ...
        def compile(self, s_optimizer, w_optimizer, loss):
            super(KINT, self).compile()
        ...
        def train_step(self, data):
            ...
            choice = self.selector(src, training=False)

            if tf.argmax(choice, 1) == tf.constant(0, dtype=tf.int64):
                with tf.GradientTape() as tape_w:
                    output = self.w1(src)
                    loss_w = self.loss_fn(trg, output)
                grads_w = tape_w.gradient(loss_w, self.w1.trainable_weights)
                self.w_optimizer.apply_gradients(zip(grads_w, self.w1.trainable_weights))
                with tf.GradientTape() as tape_s:
                    choice = self.selector(src)
                    loss_s = self.loss_fn(trg, output + 0 * choice)
                grads_s = tape_s.gradient(loss_s, self.selector.trainable_weights)
                self.s_optimizer.apply_gradients(zip(grads_s, self.selector.trainable_weights))

                return {"loss_way": loss_w, "loss_selector": loss_s}

            if tf.argmax(choice, 1) == tf.constant(1, dtype=tf.int64):
                with tf.GradientTape() as tape_w:
                    output = self.w2(src)
                    loss_w = self.loss_fn(trg, output)
                grads_w = tape_w.gradient(loss_w, self.w2.trainable_weights)
                self.w_optimizer.apply_gradients(zip(grads_w, self.w2.trainable_weights))
                with tf.GradientTape() as tape_s:
                    choice = self.selector(src)
                    loss_s = self.loss_fn(trg, output + 0 * choice)
                grads_s = tape_s.gradient(loss_s, self.selector.trainable_weights)
            ...
            if tf.argmax(choice, 1) == tf.constant(2, dtype=tf.int64):
                with tf.GradientTape() as tape_w:
                    output = self.w3(src)
                    loss_w = self.loss_fn(trg, output)
                grads_w = tape_w.gradient(loss_w, self.w3.trainable_weights)
                self.w_optimizer.apply_gradients(zip(grads_w, self.w3.trainable_weights))
                with tf.GradientTape() as tape_s:
                    choice = self.selector(src)
                    loss_s = self.loss_fn(trg, output + 0 * choice)
                grads_s = tape_s.gradient(loss_s, self.selector.trainable_weights)
            ...
            if tf.argmax(choice, 1) == tf.constant(3, dtype=tf.int64):
                with tf.GradientTape() as tape_w:
                    output = self.w4(src)
                    loss_w = self.loss_fn(trg, output)
                grads_w = tape_w.gradient(loss_w, self.w4.trainable_weights)
                self.w_optimizer.apply_gradients(zip(grads_w, self.w4.trainable_weights))
                with tf.GradientTape() as tape_s:
                    choice = self.selector(src)
                    loss_s = self.loss_fn(trg, output + 0 * choice)
                grads_s = tape_s.gradient(loss_s, self.selector.trainable_weights)
            ...
            if tf.argmax(choice, 1) == tf.constant(4, dtype=tf.int64):
                with tf.GradientTape() as tape_w:
                    output = self.w5(src)
                    loss_w = self.loss_fn(trg, output)
                grads_w = tape_w.gradient(loss_w, self.w5.trainable_weights)
                self.w_optimizer.apply_gradients(zip(grads_w, self.w5.trainable_weights))
                with tf.GradientTape() as tape_s:
                    choice = self.selector(src)
                    loss_s = self.loss_fn(trg, output + 0 * choice)
                grads_s = tape_s.gradient(loss_s, self.selector.trainable_weights)
            ...
            if tf.argmax(choice, 1) == tf.constant(5, dtype=tf.int64):
                with tf.GradientTape() as tape_w:
                    output = self.w6(src)
                    loss_w = self.loss_fn(trg, output)
                grads_w = tape_w.gradient(loss_w, self.w6.trainable_weights)
                self.w_optimizer.apply_gradients(zip(grads_w, self.w6.trainable_weights))
                with tf.GradientTape() as tape_s:
                    choice = self.selector(src)
                    loss_s = self.loss_fn(trg, output + 0 * choice)
                grads_s = tape_s.gradient(loss_s, self.selector.trainable_weights)
            ...
            if tf.argmax(choice, 1) == tf.constant(6, dtype=tf.int64):
                with tf.GradientTape() as tape_w:
                    output = self.w7(src)
                    loss_w = self.loss_fn(trg, output)
                grads_w = tape_w.gradient(loss_w, self.w7.trainable_weights)
                self.w_optimizer.apply_gradients(zip(grads_w, self.w7.trainable_weights))
                with tf.GradientTape() as tape_s:
                    choice = self.selector(src)
                    loss_s = self.loss_fn(trg, output + 0 * choice)
                grads_s = tape_s.gradient(loss_s, self.selector.trainable_weights)
            ...
            if tf.argmax(choice, 1) == tf.constant(7, dtype=tf.int64):
                with tf.GradientTape() as tape_w:
                    output = self.w8(src)
                    loss_w = self.loss_fn(trg, output)
                grads_w = tape_w.gradient(loss_w, self.w8.trainable_weights)
                self.w_optimizer.apply_gradients(zip(grads_w, self.w8.trainable_weights))
                with tf.GradientTape() as tape_s:
                    choice = self.selector(src)
                    loss_s = self.loss_fn(trg, output + 0 * choice)
                grads_s = tape_s.gradient(loss_s, self.selector.trainable_weights)
            ...
            if tf.argmax(choice, 1) == tf.constant(8, dtype=tf.int64):
                with tf.GradientTape() as tape_w:
                    output = self.w9(src)
                    loss_w = self.loss_fn(trg, output)
                grads_w = tape_w.gradient(loss_w, self.w9.trainable_weights)
                self.w_optimizer.apply_gradients(zip(grads_w, self.w9.trainable_weights))
                with tf.GradientTape() as tape_s:
                    choice = self.selector(src)
                    loss_s = self.loss_fn(trg, output + 0 * choice)
                grads_s = tape_s.gradient(loss_s, self.selector.trainable_weights)
            ...
            if tf.argmax(choice, 1) == tf.constant(9, dtype=tf.int64):
                with tf.GradientTape() as tape_w:
                    output = self.w10(src)
                    loss_w = self.loss_fn(trg, output)
                grads_w = tape_w.gradient(loss_w, self.w10.trainable_weights)
                self.w_optimizer.apply_gradients(zip(grads_w, self.w10.trainable_weights))
                with tf.GradientTape() as tape_s:
                    choice = self.selector(src)
                    loss_s = self.loss_fn(trg, output + 0 * choice)
                grads_s = tape_s.gradient(loss_s, self.selector.trainable_weights)
    ...
    if settings.boot_from_file:
        ...
        model.compile(s_optimizer=tf.keras.optimizers.Adam(learning_rate=0.00001), w_optimizer=tf.keras.optimizers.Adam(learning_rate=0.00001), loss=tf.keras.losses.MeanSquaredError()) #  metrics=[TruePositivesM()]
        ...
        model((tf.keras.Input(shape=(500, 5)), tf.keras.Input(shape=(1, 10))))
        model.fit(train_d, epochs=100)
        print(round(model.optimizer.lr.numpy(), 5))
