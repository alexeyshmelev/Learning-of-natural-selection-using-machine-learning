...
def INT():
        ...
        def __init__(self, num, path, type, batch_size, from_file=False):
            ...
            if from_file:
                with open(path + 'all_inputs_{}.npy'.format(type), 'rb') as f:
                    ...
                    self.copy = np.all(self.all_inputs[:, :, 0].squeeze() != 0, axis=1)
    ...
    if settings.boot_from_file:
        ...
        valid_d = DataGen(4344, r'C:\HSE\EPISTASIS\nn\\', 'test', 32, True)
        print("Making training dataset...", flush=True)
        train_d = DataGen(49263, r'C:\HSE\EPISTASIS\nn\old_data\\', 'train', 32, True)
