...
def INT():
    ...
    class ConvBlock(tf.keras.layers.Layer):
        def __init__(self):
            ...
            self.conv1 = tf.keras.layers.Conv1D(64, 1, use_bias=False, padding="same", kernel_initializer="he_normal", kernel_regularizer=tf.keras.regularizers.l2(1e-4))
            self.conv2 = tf.keras.layers.Conv1D(64, 5, use_bias=False, padding="same", kernel_initializer="he_normal", kernel_regularizer=tf.keras.regularizers.l2(1e-4))
    ...
    class TransitionBlock(tf.keras.layers.Layer):
        def __init__(self):
            ...
            self.conv = tf.keras.layers.Conv1D(64, 1, use_bias=False, padding="same", kernel_initializer="he_normal", kernel_regularizer=tf.keras.regularizers.l2(1e-4))
            self.pool = tf.keras.layers.MaxPooling1D(pool_size=2, strides=2)


        def call(self, src):

            output = self.batch(src)
    ...
    class DenseBlock(tf.keras.layers.Layer):
        ...
        def call(self, src):
            ...
            for i in range(4):
                output = self.blocks[i](src)
                src = tf.concat([src, output], axis=-1)
    ...
    class KINT(tf.keras.Model):
        def __init__(self):
            ...
            self.conv1 = tf.keras.layers.Conv1D(64, 10, use_bias=False, padding="same", kernel_initializer="he_normal", kernel_regularizer=tf.keras.regularizers.l2(1e-4))
            ...
            for i in range(4):
                ...
                if i+1 < 4:
                    self.transition_blocks.append(TransitionBlock())

            self.last_pool = tf.keras.layers.MaxPooling1D(pool_size=2, strides=2)
        ...
        def call(self, src):
            ...
            for i in range(3):
                ...
                output = self.transition_blocks[i](output)

            output = self.dense_blocks[3](output)
            output = self.batch2(output)
            output = tf.nn.relu(output)
    ...
    if settings.boot_from_file:
        ...
        valid_d = DataGen(199, r'##########', 'test', 32, True)
        print("Making training dataset...", flush=True)
        train_d = DataGen(111993, r'##########', 'train', 32, True)
