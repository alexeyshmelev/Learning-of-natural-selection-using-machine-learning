...
def INT():
    ...
    class DataGen(tf.keras.utils.Sequence):
        ...
        def __getitem__(self, index):
            ...
            for s in target:
                if np.argmax(s) != 0:
                    s[np.argmax(s)-1:np.argmax(s)+2:2] = 0
                else:
                    s[np.argmax(s):np.argmax(s)+2] = 0
                s[np.argmax(s)] = 1
            # tf.print(target)
    ...
    class Transformer(tf.keras.layers.Layer):
        def __init__(self):
            ...
            self.attention1 = tf.keras.layers.MultiHeadAttention(num_heads=2, key_dim=10, dropout=0.7)
            self.drop1 = tf.keras.layers.Dropout(0.7)
            self.batch1 = tf.keras.layers.BatchNormalization()
            self.attention2 = tf.keras.layers.MultiHeadAttention(num_heads=2, key_dim=10, dropout=0.7)
            self.drop2 = tf.keras.layers.Dropout(0.7)
            self.batch2 = tf.keras.layers.BatchNormalization()
            self.dense1 = tf.keras.layers.Dense(200, activation="relu")
            self.dense2 = tf.keras.layers.Dense(5)
            self.drop3 = tf.keras.layers.Dropout(0.7)
            self.drop4 = tf.keras.layers.Dropout(0.7)
    ...
    class KINT(tf.keras.Model):
        ...
        def call(self, src):
            ...
            output = self.transformer1(src)
            output = self.transformer2(output)

            output = tf.reshape(output, [32, 2500])
...
def test():
    ...
    class KINT(tf.keras.Model):
        def __init__(self):
            ...
            self.conv1 = tf.keras.layers.Conv1D(64, 9, padding='same')
            self.conv2 = tf.keras.layers.Conv1D(64, 9, padding='same')
            self.conv3 = tf.keras.layers.Conv1D(64, 9, padding='same')
            self.conv4 = tf.keras.layers.Conv1D(64, 9, padding='same')
            self.conv5 = tf.keras.layers.Conv1D(64, 9, padding='same')
            self.conv6 = tf.keras.layers.Conv1D(64, 9, padding='same')
            self.conv7 = tf.keras.layers.Conv1D(64, 9, padding='same')
            self.conv8 = tf.keras.layers.Conv1D(64, 9, padding='same')
    ...
    with tf.device('/cpu:0'):
        ...
        model.summary()
        model.load_weights("weights.0510.hdf5")
        print("Making testing dataset...", flush=True)
