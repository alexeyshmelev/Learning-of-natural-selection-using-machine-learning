...
for i in range(1429):
    for gen in range(1, 2, 100):
        ...
        my_file.write("S\tF\t0\t.00515\t{}\t{}\t1".format(1 - frc, 1 - frc / 2))
        my_file.close()
    
        filename = str(gen) + '_' + str(adm) + '_' + str(frc) + '_0.txt'
    
        if os.path.exists('../next_gen_simulation_real/' + filename):
            j = 1
            while os.path.exists('../next_gen_simulation_real/' + filename):
